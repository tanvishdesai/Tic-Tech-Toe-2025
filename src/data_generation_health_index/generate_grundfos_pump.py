...
# --- Configuration ---
MACHINE_TYPE = "grundfos_pump" # <-- Changed
TARGET_ROWS = 200_000
...
def apply_degradation(timestamps, data, failure_pattern, segment_duration_steps, machine_type):
    # ... (Function body is now identical to the fixed one) ...
    """
...
if __name__ == "__main__":
    ...
    current_time = datetime.now() - timedelta(days=int(TARGET_ROWS * SAMPLING_INTERVAL_HOURS / 24)+1)
    ...
    while total_rows_generated < TARGET_ROWS:
        ...
        # Grundfos Pump: Demand fluctuation pattern on flow rate and power
        segment_data = apply_cyclical_pattern(timestamps, segment_data, "flow_rate",
                                              amplitude=0.2, period_hours=12, start_date=global_start_time)
        segment_data = apply_cyclical_pattern(timestamps, segment_data, "power",
                                              amplitude=0.15, period_hours=12, start_date=global_start_time)
        # ------------------------------------------------- #
        ...
        df_segment = df_segment.reindex(columns=cols)
